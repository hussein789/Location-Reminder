...
class GeofenceTransitionsJobIntentService : JobIntentService(), CoroutineScope {
    ...
    override fun onHandleWork(intent: Intent) {
        ...
        if(intent.action == SaveReminderFragment.ACTION_GEOFENCE_EVENT){
            ...
            if(geofenceEvent.hasError()){
                Log.d("hussein","error happened due to ${geofenceEvent.errorCode}")
                return
            }
    ...
    private fun sendNotification(triggeringGeofences: List<Geofence>) {
        triggeringGeofences.forEach { currentTriggeringGeofence ->
            val requestId = currentTriggeringGeofence.requestId
